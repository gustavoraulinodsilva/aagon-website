    ...
    <div class="bg-slate-950 pb-24 pt-28 md:pt-36" data-about-page>
        <section class="mx-auto grid w-full max-w-7xl gap-10 px-6 md:grid-cols-[1.1fr_1fr] md:items-center md:px-8">
            <div class="space-y-6">
                ...
                <h1 class="reveal translate-y-6 text-4xl font-semibold leading-tight tracking-tight text-slate-50 opacity-0 transition duration-700 sm:text-5xl" data-reveal data-reveal-delay="100">
                ...
                <p class="reveal max-w-2xl translate-y-6 text-base leading-relaxed text-slate-300 opacity-0 transition duration-700" data-reveal data-reveal-delay="180">
                    A Aagon combina estrategia e engenharia para transformar desafios complexos em produtos digitais claros, eficientes e escalaveis para diferentes segmentos empresariais.
                </p>
            </div>

            <figure class="reveal relative translate-y-6 overflow-hidden rounded-3xl border border-cyan-900/40 opacity-0 transition duration-700" data-reveal data-reveal-delay="220">
                <img
                    src="{{ asset('images/aboutimageaagon.jpeg') }}"
                    alt="Equipe da Aagon em reuniao estrategica"
                    class="h-95 w-full object-cover md:h-115"
                    loading="lazy"
                >
                <figcaption class="absolute inset-x-4 bottom-4 rounded-xl border border-slate-300/20 bg-slate-950/65 px-4 py-3 text-sm text-slate-100 backdrop-blur">
        ...
        <section class="mx-auto mt-20 w-full max-w-7xl px-6 md:px-8">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                @foreach ($approachPillars as $pillar)
                    <article class="reveal translate-y-6 rounded-2xl border border-slate-800 bg-slate-900/70 p-6 opacity-0 transition duration-700 hover:border-cyan-300/30" data-reveal data-reveal-delay="{{ 100 + ($loop->index * 90) }}">
        ...
        <section class="mx-auto mt-20 w-full max-w-7xl px-6 md:px-8">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                @foreach ($workflowSteps as $step)
                    <article class="reveal translate-y-6 rounded-2xl border border-amber-200/15 bg-slate-900/80 p-6 opacity-0 transition duration-700 hover:border-amber-200/35" data-reveal data-reveal-delay="{{ 120 + ($loop->index * 80) }}">
        ...
        <section class="mx-auto mt-20 w-full max-w-7xl px-6 md:px-8">
            <div class="grid gap-10 rounded-3xl border border-cyan-900/30 bg-slate-900/70 p-8 md:grid-cols-[1fr_1.2fr] md:p-10">
                <div>
                    <p class="reveal translate-y-6 text-xs font-semibold uppercase tracking-[0.22em] text-cyan-200 opacity-0 transition duration-700" data-reveal>
                        Valores
                    </p>
                    <h2 class="reveal mt-3 translate-y-6 text-3xl font-semibold tracking-tight text-slate-50 opacity-0 transition duration-700" data-reveal data-reveal-delay="90">
                ...
                <ul class="grid gap-3 sm:grid-cols-2">
                    @foreach ($values as $value)
                        <li class="reveal flex translate-y-6 items-center rounded-xl border border-slate-800 bg-slate-950/50 px-4 py-3 text-sm text-slate-200 opacity-0 transition duration-700" data-reveal data-reveal-delay="{{ 110 + ($loop->index * 70) }}">
        ...
        <section class="mx-auto mt-20 w-full max-w-7xl px-6 md:px-8">
            ...
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                @foreach ($numbers as $item)
                    <article class="reveal translate-y-6 rounded-2xl border border-slate-800 bg-slate-900/70 p-6 text-center opacity-0 transition duration-700" data-reveal data-reveal-delay="{{ 120 + ($loop->index * 80) }}">
